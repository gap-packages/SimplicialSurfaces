\documentclass{article}

\input{../TikZHeader.tex}
\usetikzlibrary{arrows, decorations.pathreplacing, calc, decorations.pathmorphing, shapes}

\usepackage{amssymb}


%%% Vertex styles

% Courtesy of
% https://tex.stackexchange.com/questions/59354/no-text-none-in-tikz
\makeatletter
\newif\iftikz@node@phantom
\tikzset{
  phantom/.is if=tikz@node@phantom,
  text/.code=%
    \edef\tikz@temp{#1}%
    \ifx\tikz@temp\tikz@nonetext
      \tikz@node@phantomtrue
    \else
      \tikz@node@phantomfalse
      \let\tikz@textcolor\tikz@temp
    \fi
}
\usepackage{etoolbox}
\patchcmd\tikz@fig@continue{\tikz@node@transformations}{%
  \iftikz@node@phantom
    \setbox\pgfnodeparttextbox\hbox{}
  \fi\tikz@node@transformations}{}{}
\makeatother


%%% Edge styles
\tikzset{ edgePlain/.style={line join=round} }
\tikzset{ edgeDouble/.style={thin, double=gray!90!white, double distance=.3pt, line join=round} }
\tikzset{ edgeDefault/.style={edgePlain} }
\tikzset{ edge/.code={
    \ifdefined\edgeLabels
        \tikzset{fill=blue!20!white, edgeDefault}
    \else
        \tikzset{edgeDefault}
    \fi}
}
\newcommand{\edge}[1]{
    \ifdefined\edgeLabels #1 \fi
}


%%% Face styles
\tikzset{ face/.style={fill=#1} }

\newcommand{\faceColorY}{yellow!60!white}
\tikzset{ faceY/.style={face=\faceColorY} } % yellow 
\newcommand{\faceColorB}{blue!60!white}
\tikzset{ faceB/.style={face=\faceColorB} } % blue 
\tikzset{ faceP/.style={face=cyan!60} } % purple
\tikzset{ faceR/.style={face=red!60!white} } % red
\tikzset{ faceG/.style={face=green!60!white} } % green
\tikzset{ faceO/.style={face=orange!50!yellow!70!white} } % orange

\tikzset{ face/.default={\faceColorY} }
\tikzset{ faceSwap/.code=
    \ifdefined\swapColours
        {face=\faceColorB}
    \else
        {face=\faceColorY}
    \fi
}


% Recursive style definition idea from
% https://tex.stackexchange.com/questions/55948/choosing-styles-conditionally-in-tikz
% It is coupled with ideas from
% https://tex.stackexchange.com/questions/125676/key-that-takes-a-list-of-other-keys-as-argument-and-sets-them
\tikzset{ 
    vertexNodePlain/.style = {fill=gray, shape=circle, inner sep=0pt, minimum size=2pt, text=none},
    vertexPlain/labels/.style = {
        vertexNode/.style={vertexNodePlain},
        vertexLabel/.style={gray}
    },
    vertexPlain/nolabels/.style = {
        vertexNode/.style={vertexNodePlain},
        vertexLabel/.style={text=none}
    },
    vertexPlain/.style = vertexPlain/#1,
    vertexPlain/.default=labels
}
\tikzset{
    vertexNodeNormal/.style = {fill=blue, shape=circle, inner sep=0pt, minimum size=4pt, text=none},
    vertexNormal/labels/.style = {
        vertexNode/.style={vertexNodeNormal},
        vertexLabel/.style={blue}
    },
    vertexNormal/nolabels/.style = {
        vertexNode/.style={vertexNodeNormal},
        vertexLabel/.style={text=none}
    },
    vertexNormal/.style = vertexNormal/#1,
    vertexNormal/.default=labels
}
\tikzset{
    vertexNodeBall/.style = {shape=circle, ball color=orange, inner sep=2pt, outer sep=0pt, minimum size=3pt},
    vertexBall/labels/.style = {
        vertexNode/.style={vertexNodeBall, text=black},
        vertexLabel/.style={text=none}
    },
    vertexBall/nolabels/.style = {
        vertexNode/.style={vertexNodeBall, text=none},
        vertexLabel/.style={text=none}
    },
    vertexBall/.style = vertexBall/#1,
    vertexBall/.default=labels
}
\tikzset{ 
    vertexDefault/.style={vertexNormal=#1},
    vertexDefault/.default = labels
}

\begin{document}


\begin{tikzpicture}[edgeDefault/.style=edgeDouble, vertexDefault=nolabels]

    \coordinate (A) at (0,0);
    \coordinate (B) at (1,0);
    \coordinate (C) at (1,3);
    \coordinate (D) at (4,2);

    \draw[edgePlain] (A) -- (B);
    \draw[edgeDouble] (B) -- (C);
    \def\edgeLabels{1}
    \draw[edge] (C) to node[edge] {\edge{2}} (D);

    \node[vertexNode] at (A) {5};
    \node[vertexNode] at (B) {2};
    \node[vertexNode] at (C) {8};
    \node[vertexNode] at (D) {9};
    \node[vertexLabel] at (5,0) {10};

\end{tikzpicture}





\end{document} 
