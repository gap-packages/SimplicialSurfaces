...
\begin{tikzpicture}[edgeLabel/.style={fill=red!10!white}]
    ...
    \ifdefined \edgeLabels
        \node[edgeLabel] at ($1/2*(A)+1/2*(B)$) {1};
        \node[edgeLabel] at ($1/2*(A)+1/2*(C)$) {2};
        \node[edgeLabel] at ($1/2*(C)+1/2*(B)$) {3};
        \node[edgeLabel] at ($1/2*(D)+1/2*(B)$) {4};
        \node[edgeLabel] at ($1/2*(E)+1/2*(B)$) {5};
        \node[edgeLabel] at ($1/2*(C)+1/2*(D)$) {6};
        \node[edgeLabel] at ($1/2*(D)+1/2*(E)$) {7};
    \fi
\end{tikzpicture}
