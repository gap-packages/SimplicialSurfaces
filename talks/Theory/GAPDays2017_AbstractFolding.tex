...
\begin{frame}[fragile]{Structure of multiple foldings}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            % Draw a plane behind the two parameters and an edge behind the first one
            \newcommand{\plane}[2]{
                \filldraw[fill=black!20!white,draw=black!50!white] 
                    (#1) -- (#2) -- ($(#2)+(Back)$) -- ($(#1)+(Back)$) -- cycle;
                \draw (#1) -- (#2);
            }
            ...
            \begin{scope}
                ...
                \uncover<5->{
                    \planEdge{\colorRed}{($(120+\off:0.5*\len)+0.5*(Back)$)}
                        {($(180:0.5*\len)+0.5*(Back)$)}{($(-120-\off:0.5*\len)+0.5*(Back)$)}
                }
            \end{scope}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \begin{scope}
                ...
                \uncover<5->{
                    \planEdge{\colorRed}{(120-\off:0.5*\len)}{(60:0.5*\len)}{(\off:0.5*\len)}
                }
            \end{scope}
