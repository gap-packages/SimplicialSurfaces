        ...
        \begin{center}
            ...
            \only<11-13|handout:1>{
                \begin{tikzpicture}
                    ...
                    \draw (A) -- (B) -- (C) -- (D) -- cycle;
                    \uncover<11>{
                        \draw[\colorRed] (A) -- (60:\fak*\len);
                        \draw[\colorBlue] (B) -- ($(B)+(120:\fak*\len)$);
                    }

                    \draw[<->] ($(A)!0.5!(D)$) to[bend right] ($(D)!0.5!(C)$);

                    \uncover<12|handout:0>{
                        \draw[\colorRed] (A) -- (5:\fak*\len);
                        \draw[\colorBlue] (B) -- ($(B)+(160:\fak*\len)$);
                    }

                    \uncover<13|handout:0>{
                        \draw[\colorRed] (A) -- (20:\fak*\len);
                        \draw[\colorBlue] (B) -- ($(B)+(175:\fak*\len)$);
                    }
                \end{tikzpicture}
            }
            % 11: general picture
