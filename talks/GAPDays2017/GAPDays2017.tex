...
\begin{frame}{Motivation}
    ...
    \only<2->{
        ...
        \end{center}
        \begin{itemize}
            \item<9-> embeddings are difficult to compute
                \begin{itemize}
                    \item<10-> some embeddings of an asymmetric icosahedron are not feasible to compute
                \end{itemize}
            \item<11->[$\leadsto$] focus on intrinsic properties
            \item<12->[$\leadsto$] incidence geometry
        \end{itemize}
    }
\end{frame}
...
\begin{frame}{Implementation in GAP}
        \begin{itemize}
            ...
            \item<6-> difference to \texttt{FinInG}--package by De Beule, Neunhöffer et al.
                \begin{itemize}
                    \item<7-> only two dimensions \uncover<8->{but it can work with colourings and foldings}
                \end{itemize}
        \end{itemize}
\end{frame}
        ...
        \begin{minipage}{\textwidth}
            \begin{minipage}{\links\textwidth}
                ...
                \begin{itemize}
                    ...
                    \item<6-> a lot of theory missing
                \end{itemize}
            \end{minipage}
            \begin{minipage}{\rechts\textwidth}
                \uncover<4->{
