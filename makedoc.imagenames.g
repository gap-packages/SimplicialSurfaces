...
preProcessTikz := function( node )
    local cont, n1, n2, n3, file, output, name, htmlString, consoleString, 
        ...
        hash, tmpImageName, latexString, i, tmphash;
    ...
    if node.name = "Alt" and IsBound(node.attributes.Only) and 
        ...
        name := SplitString(cont,"{","}");
        i := fail;
        for tmphash in name do
            if Length(tmphash)> 4 and tmphash[Length(tmphash)-2] = 't' and
               tmphash[Length(tmphash)-1] = 'e' and
               tmphash[Length(tmphash)] = 'x' and 
               tmphash[Length(tmphash)-3] = '.' then 
	       i := tmphash;
               break;
            fi;
         od;
         if i <> fail then 
             name := i{[1..Length(i)-4]}; 
             name := Concatenation( "_Wrapper_", name );
         else
             name := Concatenation( "_IMAGE_", hash );
             #Error("+++++++++++Name does not appear to be a tex file" );
        fi;
Print("======================>", name,"\n");
